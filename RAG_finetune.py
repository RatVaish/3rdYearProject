...
one_problem_statement = df["problem_statement"].iloc[0]
nlp = spacy.load('en_core_web_sm')

keywords = extract_keywords(one_problem_statement)

code_files = search_github_repository(keywords)

if not code_files:
    print(f"No relevant code snippets found ")

best_match = find_relevant_code_snippets(one_problem_statement, code_files)

print(f"Most relevant file for issue : {best_match}")
